...
function Player:update()
    ...
    local cameraOffsetX = math.max(math.min(idealX, levelGX + levelWidth - 400), levelGX)
    local cameraOffsetY = math.max(math.min(idealY, levelGY + levelHeight - 240), levelGY)
    ...
    if playerX > levelGX + levelWidth then
        direction = DIRECTION.RIGHT
    elseif playerX < levelGX then
        direction = DIRECTION.LEFT
    end

    if playerY > levelGY + levelHeight then
        direction = DIRECTION.BOTTOM
    elseif playerY < levelGY then -- Add a margin to not trigger level change so easily.
