...
function Elevator:postInit()
  ...
  -- Record initial start position so elevator can return when player steps off
  self.startPosition = { x = self.x, y = self.y }
end
...
function Elevator:update()
  ...
  -- If elevator was activated this frame (didActivate), updatePosition already handled
  -- by activateDown. If not activated and there is no child on it, move back to start.
  if not self.didActivate then
    if not self.spriteChild then
      local sx, sy = self.startPosition.x, self.startPosition.y
      if self.x ~= sx or self.y ~= sy then
        self:moveToTarget(sx, sy, (self.x ~= sx) and ORIENTATION.Horizontal or ORIENTATION.Vertical, nil, 0)
      end
    else
      self:updatePosition()
    end
